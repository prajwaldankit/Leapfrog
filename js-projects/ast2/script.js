...
function CarouselController(className) {
  this.init = function () {
    var carouselContainers = document.getElementsByClassName(className);
    var args = arguments;
    // console.log(args);
    // Array.prototype.forEach.call(carouselContainers, function (carouselContainer, index) {
    //   var width = arguments[index].width;
    //   var height = arguments[index].height;
    //   carouselContainer = new Carousel(carouselContainer).init(width, height);
    // });
    Array.from(carouselContainers).forEach(function(value, index) {
      var width = args[index]['width'];
      var height = args[index]['height'];
      new Carousel(value).init(width, height);
    })
  }
}